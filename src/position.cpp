...
bool Position::legal(Move m) const {
  ...
  if ((type_of(m) == LION || type_of(m) == HOOK) && count<KING>(us))
  {
      Square ksq = type_of(moved_piece(m)) == KING ? to : square<KING>(us);
      Bitboard occupied = (pieces() ^ from) | to;
      Square via = type_of(m) == LION ? LionVia[from][lion_path_index(m)] : hook_sq(m);
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  Square to = to_sq(m);
  Piece pc = moved_piece(m);
  Piece captured = (type_of(m) == LION || type_of(m) == HOOK) && to == from ? NO_PIECE : piece_on(type_of(m) == EN_PASSANT ? capture_square(to) : to);
  Square lionCapsq = type_of(m) == LION ? LionVia[from][lion_path_index(m)] : type_of(m) == HOOK ? hook_sq(m) : SQ_NONE;
  Piece lionCaptured = (type_of(m) == LION || type_of(m) == HOOK) && !empty(lionCapsq) && color_of(piece_on(lionCapsq)) == them ? piece_on(lionCapsq) : NO_PIECE;
  if (to == from)
  {
      assert(type_of(m) == LION || type_of(m) == HOOK || (type_of(m) == PROMOTION && sittuyin_promotion()) || (is_pass(m) && (pass(us) || var->wallOrMove )));
      captured = NO_PIECE;
  }
...
void Position::undo_move(Move m) {
  ...
  assert(type_of(m) == DROP || empty(from) || type_of(m) == CASTLING || is_gating(m) || type_of(m) == LION || type_of(m) == HOOK
